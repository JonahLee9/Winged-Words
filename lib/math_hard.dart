...
class MathHard extends StatelessWidget {
  const MathHard({super.key});
...
class _BackgroundScreenState extends State<BackgroundScreen> {
 ...
 void generateEquationAndAnswers() {
  ...
  while (true) {
    ...
    // Randomly make one of the operands negative
    int negativeIndex = random.nextInt(3);
    if (negativeIndex == 0) a *= -1;
    if (negativeIndex == 1) b *= -1;
    if (negativeIndex == 2) c *= -1;
  ...
  equation = formatEquation(a, op1, b, op2, c);
...
// Helper method to format the equation and surround negative numbers with parentheses
String formatEquation(int a, String op1, int b, String op2, int c) {
  String formatOperand(int operand) {
    return operand < 0 ? '($operand)' : operand.toString();
  }

  return '${formatOperand(a)} $op1 ${formatOperand(b)} $op2 ${formatOperand(c)} =';
}
